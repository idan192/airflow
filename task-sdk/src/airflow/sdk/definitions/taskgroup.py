...
from airflow.sdk._shared.dagnode.task_group_sort import resolve_task_group_projection_cycle
...
class TaskGroup(DAGNode):
    ...
    def topological_sort(self) -> list[DAGNode]:
        ...
        Dag task declaration order. Cycles introduced only by collapsing interleaved TaskGroups
        are ordered as strongly connected components.
        """
        ...
        def node_position(node: DAGNode) -> tuple[int, str]:
            if isinstance(node, TaskGroup):
                ...
                position = task_positions[cast("Operator", node).task_id]
        ...
        while pending:
            ...
            if len(next_pending) == len(pending):
                return resolve_task_group_projection_cycle(
                    ...
                    nodes=nodes,
                    projected=projected,
                    cycle_exception=AirflowDagCycleException,
                )
